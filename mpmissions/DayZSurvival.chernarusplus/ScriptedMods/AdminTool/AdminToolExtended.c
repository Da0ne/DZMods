		...
		if (m_VanillaLoadouts)
		    {
				...
				Msgparam = new Param1<string>( "Vanilla Loadouts DISABLED! Custom Loadouts ENABLED!" );
				GetGame().RPCSingleParam(playerAdmin, ERPCs.RPC_USER_ACTION_MESSAGE, Msgparam, true, playerAdmin.GetIdentity());
			} else {

				m_VanillaLoadouts = true;
				Msgparam = new Param1<string>( "Vanilla Loadouts ENABLED! Custom Loadouts DISABLED!" );
				GetGame().RPCSingleParam(playerAdmin, ERPCs.RPC_USER_ACTION_MESSAGE, Msgparam, true, playerAdmin.GetIdentity());
			}
	break;
