...
**DayZ Update 1.0: Fixed issues with mod startup, now functional however "StaticLoadouts/Preset" do not function due to imporper/aka borken core functionality (making folders for data to be housed). So all server owners running this mod, please use "Randomized loadouts" as till we get a fix for this meme. Other broken things include: Player Stats don't get recorded, Can't save spawnpoints from the admin tools, can't export loadouts via the admintools**
